            ...
            switch (mod) {
                ...
                case Traj1:
                    if(caz==1)
                        Actions.runBlocking(
                                drive.actionBuilder(drive.pose)
                                        .splineToLinearHeading(new Pose2d(-8, 14, Math.toRadians(115)), Math.PI / 2)
                                        .build());
                    if(caz==2)
                        Actions.runBlocking(
                                drive.actionBuilder(drive.pose)
                                        .splineToConstantHeading(new Vector2d(-5, 21.2), Math.PI / 2)
                                        .build());
                    if(caz==3)
                        Actions.runBlocking(
                                drive.actionBuilder(drive.pose)
                                        .splineToConstantHeading(new Vector2d(3, 19), Math.PI / 2)
                ...
                case TrajPre2:
                    if(caz==1)
                        ...
                        Actions.runBlocking(
                                drive.actionBuilder(drive.pose)
                                        .waitSeconds(1)
                                        .splineToLinearHeading(new Pose2d(-15, 20,Math.PI), Math.PI / 2)
                                        .waitSeconds(1)
                                        .build());
                    mod=Mod.Traj2;
                    break;
                ...
                case Traj2:
                    ...
                    Actions.runBlocking(
                            drive.actionBuilder(drive.pose)
                                    .splineToLinearHeading(new Pose2d(-39, 21, Math.PI), Math.PI / 2)
                                    .build());
                    else if(caz==2)
                        Actions.runBlocking(
                                drive.actionBuilder(drive.pose)
                                        .splineToConstantHeading(new Vector2d(-38.3, 26.8), Math.PI / 2)
                                        .turnTo(Math.PI)
                                        .build());
                    else
                        Actions.runBlocking(
                                drive.actionBuilder(drive.pose)
                                        .waitSeconds(1)
                                        .splineToConstantHeading(new Vector2d(-39, 33.5), Math.PI / 2)
                                        .build());
                    timer1.reset();
                    mod=Mod.Prep2;
                    break;
                ...
                case Park:
                    Actions.runBlocking(
                            drive.actionBuilder(drive.pose)
                                    .splineToConstantHeading(new Vector2d(-34,0), Math.PI)
                                    .build());
                    mod=Mod.Idle;
                    break;
